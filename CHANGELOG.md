...
While `MAJOR == 0`, the protocol follows the SemVer pre-1.0 convention: MINOR releases (e.g., `0.25.1` → `0.31.0`) may include breaking changes; PATCH releases within a MINOR series remain backward compatible. See [spec/versioning.format.md](./spec/versioning.format.md#pre-10-stability) for the full rules.
...
- **MINOR** (`0.25.1` → `0.31.0`): may include breaking changes
