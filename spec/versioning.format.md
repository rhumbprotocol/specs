...
- `0.28.1` - Current RWP version (pre-1.0)
- `0.28.1` - A future minor release; may include breaking changes (see [Pre-1.0 Stability](#pre-10-stability))
...
**Status**: Pre-1.0, advisory. The protocol is shaping toward a stable v1.0. Until v1.0 ships, MINOR releases (e.g., `0.28.1` → `0.28.1`) are permitted to introduce breaking changes; PATCH releases (`0.28.1` → `0.26.1`) remain backward compatible within a MINOR series.
...
|-----------|--------------|-------------------|
| PATCH (`0.28.1` → `0.26.1`) | Backward compatible. Safe to upgrade. | Backward compatible. Safe to upgrade. |
| MINOR (`0.28.1` → `0.28.1`) | **May break**. Treat as a major bump for compatibility purposes. | Backward compatible. Safe to upgrade. |
...
rwp_version: "0.28.1"    # Protocol version
created: "2026-04-27T08:00:00Z"
```
...
|------------------|-------------------------|--------|
| `0.28.1` | `0.28.1` | ✓ Full support |
| `0.26.1` | `0.28.1` | ✓ Full support (patch within same minor) |
| `0.28.1` | `0.28.1` | ✗ May fail - pre-1.0 minor bumps may break |
| `0.28.1` | `1.0.0` | Migration required - see release notes |
| `0.26.1` | `0.28.1` | ✗ Implementation predates artifact's patch only if a tool requires exact patch; RWP does not |
| `1.0.0` | `0.28.1` | ✗ Major version mismatch |
...
# Usage
assert is_compatible("0.28.1", "0.28.1")           # Same minor in 0.x
assert is_compatible("0.26.1", "0.28.1")           # Patch bump within 0.26.x
assert not is_compatible("0.28.1", "0.28.1")       # 0.x minor bumps may break
assert not is_compatible("0.28.1", "1.0.0")        # Major version mismatch
```
...
// Usage
console.assert(isCompatible("0.28.1", "0.28.1"));        // Same minor in 0.x
console.assert(isCompatible("0.26.1", "0.28.1"));        // Patch bump within 0.26.x
console.assert(!isCompatible("0.28.1", "0.28.1"));       // 0.x minor bumps may break
console.assert(!isCompatible("0.28.1", "1.0.0"));        // Major version mismatch
```
...
|---------|--------|-------|
| **0.28.1** | Current | Pre-1.0; advisory; subject to MINOR-bump breakage during 0.x maturation. |
...
Within a single MINOR series (e.g., `0.28.1` → `0.26.1`), upgrades are backward compatible. PATCH bumps fix bugs and clarify behavior; they do not change schemas or lifecycle semantics.
...
rwp_version: "v0"              # Should be "0.28.1"
rwp_version: "0"               # Should be "0.28.1"
rwp_version: "0.25"            # Should be "0.28.1"
```
...
manifest_version: "1.0"        # Manifest schema version
rwp_version: "0.28.1"          # RWP protocol version
```
...
|------|---------|---------|
| 2026-04-27 | 0.28.1 | Current version reference; pre-1.0 stability rules clarified. |
