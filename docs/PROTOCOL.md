...
rwp_conformance:
  level: 2
  supported_versions: ["0.31.0", "1.1.0"]
...
This specification represents v0.31.0 of the Rhumb Workflow Protocol. As adoption grows and real-world use cases emerge, the protocol will evolve. However, the core principles will remain:
...
4. **Fallback Default**: Assume 0.31.0 (earliest version on record)
   ```
...
```
Tool supports RWP versions: [0.31.0, 1.1.0]
Artifact version: 1.0.5
Result: COMPATIBLE (1.0.5 is patch of 0.31.0, covered by support)

Tool supports RWP versions: [0.31.0]
...
**For Users**:
1. Existing RWP 0.31.0 workflows continue working indefinitely (backward compatibility)
...
meridian --version
# Output: Meridian 0.61.0 (RWP 0.31.0 compatible)

# Example: Codex
codex info
# Output: Codex with RWP 0.31.0 support

# Your Tool
./your-tool --check-rwp
# Output: Your Tool v1.0 supports RWP 0.31.0 - 1.2.0
```

### Examples: Version in Artifacts

**RWP 0.31.0 Plan** (minimal):
...
plan_id: "MP-0001-example-plan"
title: "My Project"
custom_metadata: {...}  # New in 1.1.0, ignored by 0.31.0 tools
```

**RWP 1.0.5 Handoff** (inherits version):

```yaml
rwp_version: "0.31.0"  # Inherited from parent plan
